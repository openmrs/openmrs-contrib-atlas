...
EOL
    ...
    Log::info("Unauthorized attempt to delete ".$deleteId." from ".$_SERVER['REMOTE_ADDR']);
  }
...
if ($method != 'POST') {
  ...
  Log::info("Unauthorized $method from ".$_SERVER['REMOTE_ADDR']);
  exit;
}

$json = json_decode(Request::getContent(), true);
if (!validate($json)) {
  Log::debug("Invalid ping from ".$_SERVER['REMOTE_ADDR']);
  exit;
}
